...
describe('コーラの商品表示', () => {
    it('コーラの表示が描画されること', () => {
        ...
        expect(wrapper.find(Cola)).toHaveLength(1);
    });
})
...
describe('お茶の商品表示', () => {
    it('お茶の表示が描画されること', () => {
        const wrapper = shallow(<VendingMachine {...createProps()} />).dive();
        expect(wrapper.find(Tea)).toHaveLength(1);
    });
})
...
describe('コーヒーの商品表示', () => {
    it('コーヒーの表示が描画されること', () => {
        const wrapper = shallow(<VendingMachine {...createProps()} />).dive();
        expect(wrapper.find(Coffee)).toHaveLength(1);
    });
})
...
describe('入金処理', () => {
    ...
    it('入金すると入金フォームがクリアされること', () => {
        const wrapper = shallow(<VendingMachine  {...createProps()} />).dive();
        wrapper.find('input').simulate('change', { target: { value: '120' } });
        wrapper.find('button').simulate('click');
        expect(wrapper.find(".money-container").find("input").getElement().props.value).toEqual("");
    });
...
describe('購入済み商品表示', () => {
    ...
    it('コーラを購入すると購入済み商品エリアにコーラが1で表示されること', () => {
        ...
        wrapper.find(Cola).dive().dive().find(".item-display").find("button").simulate("click")
    ...
    it('お茶を購入すると購入済み商品エリアにお茶が1で表示されること', () => {
        ...
        wrapper.find(Tea).dive().dive().find(".item-display").find("button").simulate("click")
    ...
    it('コーヒーを購入すると購入済み商品エリアにコーヒーが1で表示されること', () => {
        ...
        wrapper.find(Coffee).dive().dive().find(".item-display").find("button").simulate("click")
